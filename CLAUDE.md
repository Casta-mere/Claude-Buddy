...
4. **Status Line** (`statusline/buddy-status.sh`) — animated display; reads TTY-scoped `~/.claude-buddy/tty-sessions/{tty}` → `sessions/{id}.json`, falling back to `status.json`. Also parses Claude Code's status JSON on stdin to render a **stacked left info bar** that fills all of the buddy's rows: identity (model + window size + thinking/effort level + active email), workspace (dir + git branch + session timer), context %, then **one row per account** showing 5h/7d rate-limit usage as block-glyph bars colored by load (green/yellow/red). Missing JSON fields collapse cleanly; rows past the buddy's art height still render (and vice-versa) so nothing is lost.
   - **Multi-account usage**: stdin only carries the *active* account's `rate_limits`, so each session writes its snapshot to a shared write-through cache `~/.claude-buddy/usage/<account>.json` (atomic temp+rename), and the bar reads every file. The active account (from `CLAUDE_CONFIG_DIR` basename) is marked `> ` and shows reset times; cached/idle accounts show an "Xago" freshness age. Account rows are ASCII-only (marker, separators, right-aligned 3-wide percentages) so columns stay aligned regardless of how a terminal renders ambiguous-width Unicode glyphs. Account labels are tinted from a cycled palette so each keeps its color.
   - `export LC_ALL=en_US.UTF-8` so `${#str}` counts display columns (Unicode block glyphs = 1 col), keeping the right-alignment width math exact. Email reads `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.claude.json` (falls back to `$HOME/.claude.json`).
